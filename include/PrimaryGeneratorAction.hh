...
class PrimaryGeneratorAction : public G4VUserPrimaryGeneratorAction
{
  ...
  public:
    void SetXmin(const G4double x){monolim_xmin = x;}
    void SetXmax(const G4double x){monolim_xmax = x;}
    void SetYmin(const G4double x){monolim_ymin = x;}
    void SetYmax(const G4double x){monolim_ymax = x;}
    ...
    void GenerateMonoLimits(G4Event* anEvent);
  ...
  protected:
    enum tBeamType : G4int {beamGauss, beamMono, beamMonoLimits, beamMC};
  ...
  private:
    ...
    G4double monolim_xmin, monolim_xmax, monolim_ymin, monolim_ymax;
